...
def train(conf):
    ...
    data_features = ['img', 'pts', 'shape_id', 'view_id']
    
    train_dataset = PartNetShapeDataset(conf.category, 'train', conf.mode, data_features)
    ...
    val_dataset = PartNetShapeDataset(conf.category, 'val', conf.mode, data_features)
...
def forward(batch, data_features, network, conf, \
    ...
    # prepare input
    input_img = torch.cat(batch[data_features.index('img')], dim=0).to(conf.device)     # B x 3 x H x W
    batch_size = input_img.shape[0]

    # forward through the network
    pred_pts = network(input_img)     # B x N x 3
    
    # prepare gt
    gt_pts = torch.cat(batch[data_features.index('pts')], dim=0).to(conf.device)        # B x N x 3

    # for each type of loss, compute losses per data
    pc_loss_per_data = network.get_pc_loss(pred_pts, gt_pts)
    
    # for each type of loss, compute avg loss per batch
    pc_loss = pc_loss_per_data.mean()
    ...
    with torch.no_grad():
        ...
        if is_val and (not conf.no_visu) and epoch % conf.num_epoch_every_visu == 0:
            ...
            gt_pts_dir = os.path.join(out_dir, 'gt_pts')
            pred_pts_dir = os.path.join(out_dir, 'pred_pts')
            info_dir = os.path.join(out_dir, 'info')

            blender_fn = 'camera_centered.blend'
            if conf.mode == 'shape_space':
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                for i in range(batch_size):
                    ...
                    render_utils.render_pts(os.path.join(BASE_DIR, pred_pts_dir, fn), cur_pred_pts, blender_fn=blender_fn)
                    cur_gt_pts = gt_pts[i].cpu().numpy()
                    render_utils.render_pts(os.path.join(BASE_DIR, gt_pts_dir, fn), cur_gt_pts, blender_fn=blender_fn)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, help='mode [shape_space or camera_space]')
    ...
    # network settings
    parser.add_argument('--decoder_type', type=str, help='decoder type')
    ...
    assert conf.mode in ['shape_space', 'camera_space'], 'mode has to be one of "shape_space" or "camera_space"!'

    ### prepare before training
    # make exp_name
    conf.exp_name = f'exp-{conf.category}-{conf.model_version}-{conf.mode}-{conf.exp_suffix}'
